...
func TestDenyAllByDefault(t *testing.T) {
	f := New()
	if f.Allow("10.7.0.2", ProtoTCP, 3389) {
		t.Fatal("deny-all must reject by default")
	}
	if f.AllowStream("10.7.0.2", "/pvn/tunnel/1.0.0") {
...
func TestAllowAll(t *testing.T) {
	...
	if !f.Allow("10.7.0.2", ProtoTCP, 1) ||
		!f.Allow("10.7.9.9", ProtoUDP, 65535) ||
		!f.AllowStream("10.7.0.2", "/pvn/tunnel/1.0.0") {
...
func TestAllowListRules(t *testing.T) {
	...
	f.Set(ModeAllowList, []Rule{
		...
		{Source: "10.7.0.5", Port: "5000-5010"},  // tcp 范围
		{Source: "10.7.1.0/24", Port: "80"},      // CIDR
		{Source: "*", Proto: ProtoUDP, Port: "53"}, // UDP DNS
		{Source: "10.7.0.7", Proto: ProtoUDP, Port: "5000-6000"},
	})
	cases := []struct {
		ip    string
		proto string
		...
		{"10.7.0.2", ProtoTCP, 3389, true},  // 通配来源（默认 tcp）
		{"10.7.0.2", ProtoTCP, 3390, false}, // 端口不匹配
		{"10.7.0.2", ProtoUDP, 3389, false}, // tcp 规则不放行 udp
		{"10.7.0.5", ProtoTCP, 5000, true},  // 范围下界
		{"10.7.0.5", ProtoTCP, 5010, true},  // 范围上界
		{"10.7.0.5", ProtoTCP, 5011, false}, // 范围外
		{"10.7.0.6", ProtoTCP, 5000, false}, // 来源不匹配
		{"10.7.1.77", ProtoTCP, 80, true},   // CIDR 内
		{"10.7.2.77", ProtoTCP, 80, false},  // CIDR 外
		{"10.7.0.2", ProtoUDP, 53, true},    // udp 规则
		{"10.7.0.2", ProtoTCP, 53, false},   // udp 规则不放行 tcp
		{"10.7.0.7", ProtoUDP, 5999, true},  // udp 范围内
		{"10.7.0.7", ProtoUDP, 6001, false}, // udp 范围外
...
func TestAllowStreamRules(t *testing.T) {
	...
	f.Set(ModeAllowList, []Rule{
		...
		{Source: "10.7.0.7", Proto: "/myapp/1.0.0"},
	})
	cases := []struct {
		ip   string
		id   string
		...
		{"10.7.0.2", "/pvn/tunnel/1.0.0", true}, // 通配来源协议规则
		{"10.7.0.2", "/myapp/1.0.0", false},     // 未配置的协议拒绝
		{"10.7.0.7", "/myapp/1.0.0", true},      // 精确来源协议规则
		{"10.7.0.9", "/myapp/1.0.0", false},     // 精确来源不匹配
		{"10.7.0.2", "tcp", false},              // 协议 ID 规则不匹配传输层字面量
	}
	for _, c := range cases {
...
func TestAllowStreamAnyProto(t *testing.T) {
	...
	if !f.AllowStream("10.7.0.2", "/anything/1.0.0") || !f.AllowStream("10.7.0.9", "/unknown/2.0") {
...
func TestSlashProtoNotTreatedAsCIDR(t *testing.T) {
	...
	if f.Allow("10.7.0.2", ProtoTCP, 80) {
