...
【常用命令行参数】
  ...
  -bootstrap         逗号分隔的成员连接种子 multiaddr（须带 /p2p/<节点ID>）；
                     留空 = 不配种子，只走私有 DHT + mDNS
  -public-dht        启用公共 DHT 临时引导（默认关闭以省流量；开启后跨网可零配置
                     冷启动，连上同群成员立即自动退出，超时未连上也自动退出）
  -public-dht-minutes 公共 DHT 最长运行分钟数，默认 10
...
【发现、路由和防火墙】
  - 默认无引导（连接种子留空）：私有 DHT + mDNS 发现，不接触任何公共设施；跨网
    冷启动推荐把已在网成员的「连接种子」（控制台「成员」页可复制）填到
    -bootstrap 或直接粘贴到控制台「直接连接」框（即时生效），
    也可临时用 -public-dht 开启公共引导（限时自动退出）。
    连接种子只接受地址：写 none / public 会被拒绝（公共引导只认 -public-dht）。
  - 控制台「成员」页可看到本机完整节点 ID 与连接种子，方便发给对方。
  - 控制台「节点配置」里的公共 DHT 开关实时反映运行状态；连上同群成员或
    超过时限后自动退出，开关随之变灰，重新勾选可即时再开（无需重启）。
