Lanet（Windows 版）—— 单一程序，客户端与服务端一体（无服务器 P2P 虚拟局域网）

【文件说明】
  lanet.exe   主程序（客户端 = 服务端，无需部署任何服务器）
  wintun.dll  虚拟网卡驱动（必须与 lanet.exe 同目录）
  README.txt  本文件

【运行要求】
  1. Windows 10 1809+ / Windows Server 2019+（x64）
  2. wintun.dll 必须与 lanet.exe 放在同一目录

【使用方法（双击即用）】
  1. 直接双击 lanet.exe，在弹出的 UAC 窗口点「是」（程序已内置管理员清单，
     无需右键"以管理员身份运行"）——没有黑框，桌面右下角出现托盘图标，
     首次启动时浏览器自动打开 Web 控制台；
  2. 在控制台「节点配置」里填写：节点名称、网络密钥（相同密钥的机器
     组成同一张私有网络），点「保存节点配置」；
  3. 重启程序（控制台右上角「重启」按钮，或托盘右键 → 退出后再双击）
     即完成入网。
  ...
  程序同目录文件（均可随 exe 一起拷贝迁移）：
    lanet.json  节点配置（名称/网络密钥/控制台地址与密码等）
...
【提示】
  ...
  3. 加入私有网络的最快方式：在 -bootstrap 里填一台已在网成员的地址；
     组内所有成员都在 NAT 后时，需要一台有公网 IP 的节点（如 VPS），
     否则会出现"能发现、连不上"。
