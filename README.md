...
命令行参数会覆盖 `lanet.json`。`bootstrap`（连接种子）只接受「种子地址」或留空，
留空即纯私有发现（私有 DHT + mDNS，不接触任何公共设施）；跨网互通需显式填成员
连接种子，或临时开启公共 DHT：
...
> `lanet.json` 带 UTF-8 BOM 也能正常解析（Windows 记事本「另存为 UTF-8」、
> PowerShell 的 `Set-Content -Encoding UTF8` 都会写 BOM）。若配置文件本身写坏了，
> 节点会**按默认值运行**并把原因写进日志，但**不会覆盖你的文件**——修好或删除后
> 重启即可（0.5.82 起；更早版本会静默改写文件，等于丢掉填过的网络密钥与控制台设置）。
...
- **公共 DHT 默认关闭（0.5.16 起）**：`enable_public_dht=true` 或 `-public-dht`
  显式开启。关闭时跨网首次牵线需用方式 ③。
- **连接种子只认地址（0.5.85 起）**：`bootstrap` 只接受逗号分隔的成员 multiaddr
  （须带 `/p2p/<节点ID>`），留空 = 不配自定义种子。历史字面量 `none` / `public`
  不再是合法值：`public` 曾在公共 DHT 同时开启时才生效，关闭时被静默丢弃，
  容易让人误以为已经挂上公共网络——公共引导现在只由方式 ④ 的开关决定。
...
控制面给每个群组分配一对网段，序号相同、成员主机号相同，因此排障时一眼能对上：
...
- 主机号取 2~254，两个版本同值（`10.7.7.5` ↔ `fd00:6c61:6e65:7::5`），每群上限 253 人；
- 客户端（Go SDK）在托管模式下直接采用控制面分配的 IPv6 配到 TUN 并写成员 /128 路由，
  与 Standalone 模式「按网络身份与 PeerID 派生」不同：后者不依赖控制面；
- 字段都是可选的：控制面不返回 `virtual_ipv6`（旧版控制面或未分配）时客户端只跑 IPv4，
  行为与升级前完全一致。
