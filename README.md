...
| 2026-09-14 | Windows + 公网 VPS（Linux 容器） | 修复 TUN 读循环遇错即退（0.5.37）；容器端口转发把虚拟网端口接到宿主服务；私有种子冷启动重连；日志自动轮转（0.5.38）；已是好友的节点不再滞留在「附近」列表（0.5.39）；服务模式身份路径漂移导致 PeerID/虚拟 IP 改变（0.5.40）；被动发现不再伪造「连接申请」（0.5.41）；「附近/待审批」不再显示本机自己、列表改为卡片式（0.5.42） |
