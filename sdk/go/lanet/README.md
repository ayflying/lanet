...
client, err := lanet.New(ctx, lanet.Config{
	...
	// 可选：私有 DHT 种子（任意已在网成员地址）；跨网冷启动可开启
	// EnablePublicDHT 或全部节点互填种子
...
- **网络即密钥**：成员发现记录的键由网络密钥派生（`SHA256`），不知道密钥
  就无法在 DHT 上定位该网络；0.5.33 起控制面协议 ID（信息交换、删除好友、
  更新分发、探测）与私有 DHT 前缀同样按群派生（`/lanet/<群指纹>/info/1.0.0`
  等），异网络节点连协议协商都过不去——密钥即私有协议准入凭证（逃生开关
  `LegacyProtocols: true` 退回历史固定 ID）；
...
- **双 DHT（私有优先 + 公共临时引导可选）**：私有网络在每台节点上同时运行两张
  完全隔离的 DHT——私有 DHT（前缀按群密钥派生，0.5.33 起形如
  `/lanet/<指纹>/kad/1.0.0`，不再全网共用固定 `/lanet/kad`）只有本网络节点参与，
